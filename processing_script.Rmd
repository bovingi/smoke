...
```{r}
par_df <- read_csv(here::here("raw-data","SMOKE DATA", "Weights_Environment", "weights.csv")) %>% 
              clean_names() %>% 
  mutate(date= as.POSIXct(date, format = "%m/%d/%y"),
         date_new = lubridate::ymd(date), 
         tree_id = as.character(tree_id)
  ...
  unite(id, c("chamber_no", "tree_id")) %>% 
  #select(date, time, id, temp, humidity, co2, par) %>% 
  mutate(day = day(date), 
         hour = hour(time))  %>% 
  separate(id, into = c("treatment", "tree_id"), sep = "_", remove = FALSE)
  

par_df %>% 
  filter(#pre_post == "post",
    day %in% c(25, 26, 29, 30), 
         hour %in% c(8:18)) %>% 
  group_by(day, treatment) %>% 
 # mutate(vpd_kpa = mean(vpd_kpa, na.rm = T)) %>% 
...
# Set the file path
file_path <- here::here("raw-data", "SMOKE DATA", "HOBOS")
...
combined_df_hobos <- dplyr::bind_rows(data_list) %>% 
  mutate(#date_time = mdy_hms(date_time), #parsing issue is the problem!
         ...
         week = week(date_time), 
         day = day(date_time)) %>% 
  separate(file_name, into = c("treatment", "tree_id", "post"), sep = "_", remove = FALSE) %>% 
  select(treatment, tree_id, date_time, month,hour, minute, time,second, week, day, temp, relative_humidity) %>% 
  drop_na(temp)   %>% 
  mutate(elev = 20) %>% 
